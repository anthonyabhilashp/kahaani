...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (logger) { logger.info(`[${scene.story_id}] 🎨 Generating image for scene ${scene.order + 1}: ${scene_id}`); }
    ...
    if (isSeriesStory) {
      if (logger) { logger.info(`[${scene.story_id}] 📺 Story belongs to series: ${story.series_id}`); }
      ...
      if (!seriesErr && series) {
        ...
        if (hasCharacterConsistency) {
          ...
          if (storyReference && storyLibrary) {
            ...
            if (logger) { logger.info(`[${scene.story_id}] 📚 Using story's own reference and library (series consistency enabled):`); }
            if (logger) { logger.info(`[${scene.story_id}]    🖼️ Reference image: ${seriesReference ? 'exists' : 'none'}`); }
            if (logger) { logger.info(`[${scene.story_id}]    👥 Character library: ${characterLibrary?.characters?.length || 0} characters`); }
          } else {
            // Fallback: load from latest story in series (for new stories)
            if (logger) { logger.info(`[${scene.story_id}] 🔍 Story has no reference yet, loading from latest story in series...`); }
            ...
            if (!latestErr && latestStory) {
              ...
              if (logger) { logger.info(`[${scene.story_id}] 📚 Loaded from latest story:`); }
              if (logger) { logger.info(`[${scene.story_id}]    🖼️ Reference image: ${seriesReference ? 'exists' : 'none'}`); }
              if (logger) { logger.info(`[${scene.story_id}]    👥 Character library: ${characterLibrary?.characters?.length || 0} characters`); }
            }
          }
        } else {
          if (logger) { logger.info(`[${scene.story_id}] ⚠️ Character consistency disabled for this series - treating stories independently`); }
        }
      }
    }
    ...
    if (!seriesReference) {
      if (storyReference) {
        if (logger) { logger.info(`[${scene.story_id}] 🖼️ Using story reference image for consistency`); }
      } else {
        ...
        if (logger) { logger.info(`[${scene.story_id}] 📸 No reference image - using ${referenceScenes.length} scene images for consistency`); }
      }
    }

    // 2️⃣ Clean up old image if exists
    if (scene.image_url) {
      if (logger) { logger.info(`[${scene.story_id}] 🧹 Removing old image from storage...`); }
    ...
    if (logger) { logger.info(`[${scene.story_id}] 📐 Story aspect ratio: ${aspect}, using ${imageSize}`); }
    if (logger) { logger.info(`[${scene.story_id}] 🧠 Using ${provider} model: ${model}`); }
...
Do NOT generate multiple scenes, panels, or images in one. Just one standalone image.`;
    ...
    // Priority 3: Fallback to scene images if no reference exists
      ...
      if (logger) { logger.info(`[${scene.story_id}] 🖼️ Including ${limitedReferences.length} scene reference images for consistency`); }
    ...
    if (logger) { logger.info(`[${scene.story_id}] 🚀 Requesting ${provider} API for scene ${scene.order + 1}...`); }
    ...
    if (!resp.ok) {
      if (logger) { logger.error(`[${scene.story_id}] ❌ API error response: ${JSON.stringify(data)}`); }
    ...
    if (logger) { logger.info(`[${scene.story_id}] 🖼️ Received image from model`); }
    ...
    if (logger) { logger.info(`[${scene.story_id}] ✅ Updated scene ${scene.order + 1} with image → ${publicUrl}`); }
    res.status(200).json({ scene_id, image_url: publicUrl, order: scene.order });

  } catch (err: any) {
    if (logger) {
      if (logger) { logger.error(`❌ Error generating scene image: ${err.message}`); }
    } else {
